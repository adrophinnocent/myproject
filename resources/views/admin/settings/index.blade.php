...
<div class="max-w-2xl mx-auto bg-white rounded-xl shadow-sm border border-gray-200 p-6">
    <form method="POST" action="{{ route('admin.settings.update') }}" enctype="multipart/form-data">
        ...
        <div class="mb-6 p-4 border border-gray-100 rounded-xl bg-gray-50">
            <label class="block text-sm font-bold text-gray-700 mb-2 uppercase tracking-wider">Interactive Safari Map Background</label>
            <div class="flex flex-col items-center justify-center border-2 border-dashed border-gray-200 rounded-2xl p-6 bg-white mb-4">
                @if(\App\Models\Setting::get('map_background'))
                    <img id="map-bg-preview" src="{{ asset('storage/' . \App\Models\Setting::get('map_background')) }}" alt="Map BG" class="h-24 mb-4 rounded shadow-sm object-cover">
                @else
                    <img id="map-bg-preview" src="" class="hidden h-24 mb-4 rounded shadow-sm object-cover">
                    <div id="map-bg-placeholder" class="text-2xl mb-2">🗺️</div>
                @endif
                <input type="hidden" name="map_background" id="map_background_path" value="{{ \App\Models\Setting::get('map_background') }}">
                <div class="flex gap-3">
                    <button type="button" onclick="window.dispatchEvent(new CustomEvent('open-media-picker', {detail: {targetId: 'map_background_path', previewId: 'map-bg-preview'}}))" class="bg-amber-600 hover:bg-amber-700 text-white px-4 py-2 rounded-lg font-bold text-[10px] uppercase tracking-widest transition-all">Choose Library</button>
                    <input type="file" name="map_background_upload" class="text-[10px] file:bg-gray-100 file:border-none file:px-3 file:py-1.5 file:rounded-md file:font-black file:uppercase">
                </div>
            </div>
        </div>
