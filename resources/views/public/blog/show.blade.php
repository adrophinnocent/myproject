            ...
            <div class="lg:col-span-8">
                ...
                <div class="mt-16 pt-10 border-t border-gray-100 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div class="flex items-center gap-4">
                        ...
                        <div>
                            <p class="text-xs text-gray-400 font-bold uppercase tracking-widest">Published By</p>
                            <p class="font-bold text-gray-900">{{ $post->author->name ?? 'Twina Safaris' }}</p>
                        </div>
                    </div>

                    <div class="flex items-center gap-4">
                        <span class="text-xs text-gray-400 font-bold uppercase tracking-widest">Share story:</span>
                        <div class="flex gap-2">
                            ...
                            <a href="https://twitter.com/intent/tweet?url={{ url()->current() }}&text={{ urlencode($post->title) }}" target="_blank" class="w-10 h-10 rounded-full border border-gray-100 flex items-center justify-center text-gray-400 hover:text-white hover:bg-black hover:border-black transition-all">
            ...
            <aside class="lg:col-span-4 space-y-12">
                ...
                <div class="bg-safari-dark rounded-3xl p-8 text-white text-center relative overflow-hidden shadow-2xl">
                    <div class="relative z-10">
                        <h3 class="font-display text-2xl font-bold mb-4">Inspired by this story?</h3>
                        <p class="text-white/60 mb-8 leading-relaxed">Let us take you to the places mentioned in this article with a custom-tailored safari experience.</p>
                        <a href="{{ route('trip-plan.index') }}" class="btn-gold w-full block py-4 rounded-xl font-bold">Start Planning</a>
                    </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <a href="{{ route('blog.show', $rel->slug) }}" class="flex gap-4 group">
                            <div class="w-24 h-20 shrink-0 rounded-xl overflow-hidden">
                                <img src="{{ $rel->featured_image_url }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" alt="{{ $rel->title }}">
                            </div>
                            <div>
...
<section class="bg-gray-50 py-20">
    <div class="max-w-4xl mx-auto px-4 text-center">
        <h2 class="font-display text-3xl md:text-4xl font-bold text-gray-900 mb-4">Stay updated with wild stories</h2>
        <p class="text-gray-600 mb-10 max-w-xl mx-auto">Join our community of travelers and receive expert tips and inspiration for your next Tanzanian journey.</p>
        <form action="{{ route('newsletter.subscribe') }}" method="POST" class="flex flex-col sm:flex-row gap-3 max-w-lg mx-auto">
            @csrf
            <input type="email" name="email" placeholder="Enter your email" class="flex-1 bg-white border border-gray-200 rounded-xl px-6 py-4 focus:outline-none focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 transition-all">
            <button class="btn-gold px-8 py-4 rounded-xl font-bold whitespace-nowrap">Join Now</button>
        </form>
    </div>
</section>
@endsection
