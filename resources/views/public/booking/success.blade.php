...
@section('title', __('Booking Confirmed') . ' - ' . (\App\Models\Setting::get('site_name', 'Twina Safaris')))
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12">
                ...
                <div class="lg:col-span-7 p-8 md:p-12 lg:border-r border-gray-100">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <div>
                            <h1 class="font-display text-3xl md:text-4xl font-bold text-safari-dark">{{ __('Booking Confirmed') }}</h1>
                            <p class="text-gold-600 font-medium tracking-wide">{{ __('Jambo') }}, {{ $booking->first_name }}! {{ __('Your adventure awaits.') }}</p>
                        </div>
                    </div>
                    ...
                    <div class="space-y-6">
                        <div class="bg-safari-dark rounded-3xl p-8 text-white relative overflow-hidden group">
                            ...
                            <div class="relative z-10">
                                <span class="text-gold-400 text-xs font-bold uppercase tracking-[0.2em] mb-2 block">{{ __('Reservation Details') }}</span>
                                <h2 class="text-2xl font-display font-bold mb-6">{{ $booking->bookable_item->translate('title') }}</h2>

                                <div class="grid grid-cols-2 gap-8">
                                    <div>
                                        <p class="text-white/40 text-[10px] font-bold uppercase tracking-widest mb-1">{{ __('Booking Reference') }}</p>
                                        <p class="text-xl font-mono font-bold text-gold-500">{{ $booking->booking_reference }}</p>
                                    </div>
                                    <div>
                                        <p class="text-white/40 text-[10px] font-bold uppercase tracking-widest mb-1">{{ __('Travel Date') }}</p>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div class="bg-white border border-gray-100 rounded-2xl p-6 shadow-sm">
                                <p class="text-gray-400 text-[10px] font-bold uppercase tracking-widest mb-1">{{ __('Guests') }}</p>
                                <p class="text-gray-900 font-bold">{{ $booking->number_of_adults }} {{ __('Adults') }} @if($booking->number_of_children > 0), {{ $booking->number_of_children }} {{ __('Children') }} @endif</p>
                            </div>
                            <div class="bg-gold-50/50 border border-gold-100 rounded-2xl p-6 shadow-sm">
                                <p class="text-gold-600/60 text-[10px] font-bold uppercase tracking-widest mb-1">{{ __('Amount Due') }}</p>
                        ...
                        <div class="flex items-start gap-3 p-4 bg-blue-50/50 border border-blue-100 rounded-2xl">
                            ...
                            <p class="text-xs text-blue-700 leading-relaxed">
                                <strong>{{ __('Urgent Note') }}:</strong> {{ __('Your booking is currently reserved for the next') }} <strong>{{ __('24 hours') }}</strong>. {{ __('Please complete the payment process below to secure your spot.') }}
                            </p>
                        </div>
                        ...
                        <div class="mt-8 border-t border-gray-100 pt-8" x-data="{ openPreview: false }">
                            <button @click="openPreview = !openPreview" class="w-full flex items-center justify-between p-4 bg-gray-50 rounded-2xl hover:bg-gold-50 transition-colors group">
                                <div class="flex items-center gap-3">
                                    ...
                                    <span class="font-bold text-gray-900">{{ __('Preview My Itinerary') }}</span>
                                </div>
                                ...
                                <div class="mt-4 space-y-4 max-h-96 overflow-y-auto pr-2 custom-scrollbar">
                                    ...
                                    @forelse($itinerary as $index => $item)
                                        <div class="flex gap-4 p-4 border-l-2 border-gold-500 bg-white">
                                            <div class="font-black text-gold-600 text-xs shrink-0">{{ __('Day') }} {{ $loop->iteration }}</div>
                                            <div>
                                    ...
                                    @empty
                                        <p class="text-gray-400 text-xs italic p-4">{{ __('Detailed day-by-day itinerary will be shared upon payment confirmation.') }}</p>
                                    @endforelse
                                </div>
                            </div>
                        </div>
                ...
                <div class="lg:col-span-5 bg-gray-50/50 p-8 md:p-12">
                    ...
                    <div class="bg-white border border-gold-500/20 rounded-3xl p-8 shadow-xl shadow-gold-500/5 mb-8 relative">
                        ...
                        <div class="space-y-6">
                            <div class="pb-4 border-b border-gray-100">
                                <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{{ __('Account Name') }}</span>
                                <span class="font-bold text-gray-900 text-lg">Twina Safaris Ltd</span>
                            </div>

                            <div class="grid grid-cols-1 gap-6">
                                <div>
                                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{{ __('Bank Name') }}</span>
                                    <span class="font-bold text-gray-800">CRDB Bank PLC</span>
                                </div>
                                <div>
                                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{{ __('Account Number (USD)') }}</span>
                                    <span class="font-bold text-gray-800 tracking-wider">0150 0000 0000</span>
                                </div>
                                <div>
                                    <span class="text-[10px] font-bold text-gray-400 uppercase tracking-widest block mb-1">{{ __('SWIFT / BIC Code') }}</span>
                            ...
                            <div class="bg-gold-500/10 p-4 rounded-xl border border-gold-500/20">
                                <p class="text-[11px] text-gold-700 font-bold leading-relaxed text-center">
                                    ⚠️ {{ __('IMPORTANT') }}: {{ __('Always include your reference') }} <span class="underline decoration-2">{{ $booking->booking_reference }}</span> {{ __('in the payment description.') }}
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="mb-10">
                        <a href="{{ route('booking.download', $booking->booking_reference) }}" class="w-full btn-gold py-5 rounded-2xl font-bold flex items-center justify-center gap-3 shadow-xl shadow-gold-500/10 hover:scale-[1.02] active:scale-95 transition-all">
                            ...
                            {{ __('Download Full Itinerary (PDF)') }}
                        </a>
                        <p class="text-center text-[10px] text-gray-400 font-bold uppercase tracking-widest mt-3">{{ __('Ready to print or save on your device') }}</p>
                    </div>

                    <div class="space-y-6">
                        <h4 class="text-sm font-bold text-safari-dark uppercase tracking-widest">{{ __('Your Next Steps') }}</h4>
                        <div class="space-y-4">
                            @foreach([
                                ['num' => '1', 'text' => __('Make payment via Bank Transfer or M-Pesa')],
                                ['num' => '2', 'text' => __('Send proof of payment via WhatsApp')],
                                ['num' => '3', 'text' => __('Receive confirmation within 1–3 hours')],
                                ['num' => '4', 'text' => __('Receive your final safari itinerary')]
                            ] as $step)
        ...
        <div class="mt-16 text-center">
            <div class="inline-block bg-white border border-gold-500/20 px-8 py-6 rounded-[2rem] shadow-xl shadow-gold-500/5 animate-pulse-soft">
                <div class="flex items-center justify-center gap-3 mb-2">
                    ...
                    <p class="text-safari-dark font-black text-sm uppercase tracking-widest">{{ __('Confirmation Sent') }}</p>
                </div>
                <p class="text-gray-500 text-sm leading-relaxed">
                    {{ __('A copy of this confirmation has been sent to') }} <span class="text-gold-600 font-black">{{ $booking->email }}</span>.<br>
                    {{ __('Please check your') }} <span class="font-bold">{{ __('inbox') }}</span> {{ __('(and spam folder) for further details.') }}
                </p>
            </div>
        </div>
