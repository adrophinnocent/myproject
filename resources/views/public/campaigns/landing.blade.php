    ...
    <div class="relative h-[70vh] flex items-center justify-center overflow-hidden bg-[#0a0703]"
        ...
        <div class="relative z-10 text-center px-4 max-w-5xl">
            ...
            <div class="text-2xl md:text-4xl text-gold-400 font-black drop-shadow-lg">
                {{ __('Exclusive Offer from') }} ${{ number_format($campaign->price, 0) }}
            </div>
            @endif
        </div>
    ...
    <div class="max-w-7xl mx-auto px-4 py-20">
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-16">
            ...
            <div class="lg:col-span-2 space-y-16">
                <div>
                    <h2 class="font-display text-4xl font-black text-gray-900 mb-8 tracking-tight border-l-4 border-gold-500 pl-6">{{ __('Adventure Overview') }}</h2>
                    <p class="text-gray-700 leading-relaxed text-xl font-light">{{ $campaign->translate('description') }}</p>
                </div>
                ...
                <div>
                    <h2 class="font-display text-4xl font-black text-gray-900 mb-8 tracking-tight border-l-4 border-gold-500 pl-6">{{ __('Experience Highlights') }}</h2>
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach(explode("\n", $campaign->translate('highlights')) as $highlight)
                            @if(trim($highlight))
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                    ...
                    <div>
                        <h2 class="font-display text-3xl font-black text-gray-900 mb-6 tracking-tight flex items-center gap-3">
                            <span class="text-green-500">✓</span> {{ __("What's Included") }}
                        </h2>
                        <ul class="space-y-3">
                            @foreach(explode("\n", $campaign->translate('inclusions')) as $inc)
                                @if(trim($inc))
                    ...
                    <div>
                        <h2 class="font-display text-3xl font-black text-gray-900 mb-6 tracking-tight flex items-center gap-3">
                            <span class="text-red-500">✕</span> {{ __("What's Excluded") }}
                        </h2>
                        <ul class="space-y-3">
                            @foreach(explode("\n", $campaign->translate('exclusions')) as $exc)
                                @if(trim($exc))
            ...
            <div class="relative">
                <div class="sticky top-28 bg-white rounded-[2.5rem] p-10 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.1)] border border-gray-100">
                    <h3 class="font-display text-3xl font-black text-gray-900 mb-2">{{ __('Claim Offer') }}</h3>
                    <p class="text-sm text-gray-500 mb-8 font-medium italic">{{ __('Available for a limited time only.') }}</p>

                    <form action="{{ route('campaign.lead', $campaign) }}" method="POST" class="space-y-5">
                        @csrf
                        <div class="space-y-1">
                            <input type="text" name="name" required placeholder="{{ __('Full Name') }}" class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-6 py-4 text-gray-900 focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 outline-none transition-all font-bold">
                        </div>
                        <div class="space-y-1">
                            <input type="email" name="email" required placeholder="{{ __('Email Address') }}" class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-6 py-4 text-gray-900 focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 outline-none transition-all font-bold">
                        </div>
                        <div class="space-y-1">
                            <input type="text" name="phone" required placeholder="{{ __('WhatsApp / Phone') }}" class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-6 py-4 text-gray-900 focus:ring-2 focus:ring-gold-500/20 focus:border-gold-500 outline-none transition-all font-bold">
                        </div>

                        <div class="grid grid-cols-2 gap-4">
                            <div class="space-y-1">
                                <label class="text-[10px] font-black uppercase text-gray-400 ml-2 tracking-widest">{{ __('Date') }}</label>
                                <input type="date" name="travel_date" class="w-full bg-gray-50 border border-gray-200 rounded-2xl px-4 py-4 text-xs font-bold">
                            </div>
                            <div class="space-y-1">
                                <label class="text-[10px] font-black uppercase text-gray-400 ml-2 tracking-widest">{{ __('Travelers') }}</label>
                    ...
                    <div class="mt-10 pt-10 border-t border-gray-100 text-center">
                        <p class="text-xs text-gray-400 mb-4 font-bold uppercase tracking-widest">{{ __('Talk to us instantly') }}</p>
    ...
    <section class="py-24 bg-gray-50 border-t border-gray-100">
        <div class="max-w-7xl mx-auto px-4">
            <div class="flex items-end justify-between mb-12">
                <div>
                    <span class="text-gold-600 text-sm font-black uppercase tracking-[0.2em] mb-3 block">{{ __('More Options') }}</span>
                    <h2 class="font-display text-4xl md:text-5xl font-black text-gray-900 leading-tight">{{ __('Recommended Trips') }}</h2>
                </div>
                    ...
                    <div class="bg-white rounded-3xl overflow-hidden border border-gray-100 shadow-sm hover:shadow-2xl transition-all duration-500 group h-full flex flex-col">
                        <div class="relative h-64 overflow-hidden">
                            ...
                            <div class="absolute top-4 left-4">
                                <span class="px-3 py-1 rounded-full bg-white/90 backdrop-blur-sm text-safari-dark text-[10px] font-black uppercase tracking-widest">{{ __($tour->category->name ?? 'Safari') }}</span>
                            </div>
                            <div class="absolute bottom-4 right-4">
                                <span class="px-4 py-2 rounded-xl bg-gold-500 text-safari-dark font-black text-sm shadow-xl">{{ __('From') }} {{ $tour->formatted_price }}</span>
                            </div>
                        </div>
                        <div class="p-8 flex-grow flex flex-col">
                            ...
                            <div class="flex items-center gap-4 text-gray-400 text-[10px] font-black uppercase tracking-widest mb-6">
                                ...
                                <span>{{ $tour->destination?->translate('name') ?? __('Tanzania') }}</span>
                            </div>
                            <div class="mt-auto">
