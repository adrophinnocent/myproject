...
<section class="relative bg-safari-dark text-white overflow-hidden">
    @if($banner && $banner->image)
        <img src="{{ asset('storage/' . $banner->image) }}" alt="{{ $banner->title }}" class="absolute inset-0 w-full h-full object-cover opacity-40">
    @endif
    ...
    <div class="relative max-w-7xl mx-auto px-4 py-24">
        <div class="max-w-2xl">
            <span class="text-gold-400 text-sm font-semibold uppercase tracking-widest">Help Center</span>
            <h1 class="font-display text-4xl md:text-6xl font-bold mt-4 mb-6">{{ $banner->title ?? 'Frequently Asked Questions' }}</h1>
            @if($banner && $banner->description)
                <p class="text-xl text-gray-300">{{ $banner->description }}</p>
            @endif
        </div>
    </div>
</section>
...
<section class="py-20 bg-gray-50">
    <div class="max-w-4xl mx-auto px-4">
        <div x-data="{ openFaq: null }">
            @forelse($faqs as $faq)
                <div class="mb-6 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                    ...
                    >
                        <span class="text-lg font-semibold text-safari-dark">{{ $faq->question }}</span>
        ...
        <div class="mt-16 text-center p-8 bg-gradient-to-r from-gold-100 to-gold-50 rounded-xl border border-gold-200">
            <h3 class="font-display text-2xl text-safari-dark mb-3">Still have questions?</h3>
            <p class="text-gray-600 mb-6">Can't find the answer you're looking for? Feel free to contact us directly.</p>
