...
<div class="max-w-7xl mx-auto px-4 py-12">
    <div class="flex flex-col lg:flex-row gap-12">
        <div class="flex-1">
            ...
            <div class="mb-10">
                ...
                <div class="prose prose-gray max-w-none text-gray-600 leading-relaxed">{!! $tour->translate('description') !!}</div>
            </div>
            ...
            @if($tour->video_url)
            <div class="mb-10">
                <h2 class="font-display text-2xl font-semibold text-gray-900 mb-4">{{ __('Tour Video') }}</h2>
            ...
            <div class="mb-12 bg-gray-200/50 rounded-3xl p-8 md:p-12 border border-gray-200">
                <h2 class="font-display text-3xl font-black text-[#e64a19] mb-8 uppercase tracking-tight">{{ __('Route summary') }}</h2>
                <div class="space-y-4">
            ...
            <div class="mb-10">
                <h2 class="font-display text-2xl font-semibold text-gray-900 mb-6">{{ __('Detailed Daily Itinerary') }}</h2>
                <div class="space-y-4">
                    @foreach($itinerary as $index => $day)
                    <div class="relative pl-8">
                        ...
                        <div class="bg-white border border-gray-100 rounded-xl p-5 shadow-sm">
                            <h4 class="font-semibold text-gray-800 mb-2">{{ __('Day') }} {{ is_numeric($index) ? $index : $loop->iteration }}: {{ $day['title'] ?? '' }}</h4>
                            ...
                            <div class="flex flex-wrap items-center gap-x-8 gap-y-3 mt-4">
                                ...
                                <div class="flex items-center gap-2 text-gray-600 whitespace-nowrap min-w-fit">
                                    ...
                                    <span class="text-sm"><span class="font-bold text-gray-900">{{ __('Stay') }}:</span> {{ is_array($day['accommodation'] ?? '') ? implode(', ', $day['accommodation']) : ($day['accommodation'] ?? '') }}</span>
                                </div>
                                @endif
                                @if(!empty($day['meals']))
                                <div class="flex items-center gap-2 text-gray-600 whitespace-nowrap min-w-fit">
                                    ...
                                    <span class="text-sm"><span class="font-bold text-gray-900">{{ __('Meals') }}:</span> {{ is_array($day['meals'] ?? '') ? implode(', ', $day['meals']) : ($day['meals'] ?? '') }}</span>
                                </div>
                                @endif
                                ...
                                <div class="flex items-center gap-2 text-gray-600 whitespace-nowrap min-w-fit">
                                    ...
                                    <span class="text-sm"><span class="font-bold text-gray-900">{{ __('Distance') }}:</span> {{ is_array($day['distance'] ?? '') ? implode(', ', $day['distance']) : ($day['distance'] ?? '') }}</span>
                                </div>
                                @endif
                                ...
                                <div class="flex items-center gap-2 text-gray-600 whitespace-nowrap min-w-fit">
                                    ...
                                    <span class="text-sm"><span class="font-bold text-gray-900">{{ __('Time') }}:</span> {{ is_array($day['hiking_time'] ?? '') ? implode(', ', $day['hiking_time']) : ($day['hiking_time'] ?? '') }}</span>
                                </div>
                                @endif
                                ...
                                <div class="flex items-center gap-2 text-gray-600 whitespace-nowrap min-w-fit">
                                    ...
                                    <span class="text-sm"><span class="font-bold text-gray-900">{{ __('Habitat') }}:</span> {{ is_array($day['habitat'] ?? '') ? implode(', ', $day['habitat']) : ($day['habitat'] ?? '') }}</span>
                                </div>
                                @endif
                                ...
                                <div class="flex items-center gap-2 text-gray-600 whitespace-nowrap min-w-fit">
                                    ...
                                    <span class="text-sm"><span class="font-bold text-gray-900">{{ __('Elevation') }}:</span> {{ is_array($day['elevation'] ?? '') ? implode(', ', $day['elevation']) : ($day['elevation'] ?? '') }}</span>
                                </div>
                                @endif
                                ...
                                <div class="flex items-start gap-2 text-gray-600 w-full mt-1">
                                    ...
                                    <span class="text-sm"><span class="font-bold text-gray-900">{{ __('Activities') }}:</span> {{ is_array($day['activities']) ? implode(', ', $day['activities']) : $day['activities'] }}</span>
                                </div>
                                @endif
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-10">
                ...
                <div class="bg-green-50 border border-green-200 rounded-2xl p-6">
                    <h3 class="font-semibold text-gray-800 mb-4 flex items-center gap-2"><span class="text-green-500">✓</span> {{ __("What's Included") }}</h3>
                    <ul class="space-y-2">
                        ...
                        <li class="flex items-start gap-3 text-gray-600 text-sm">
                            ...
                            {{ is_array($item) ? implode(', ', $item) : __($item) }}
                        </li>
                        @endforeach
                    </ul>
                </div>
                ...
                <div class="bg-red-50 border border-red-200 rounded-2xl p-6">
                    <h3 class="font-semibold text-gray-800 mb-4 flex items-center gap-2"><span class="text-red-400">✗</span> {{ __('Not Included') }}</h3>
                    <ul class="space-y-2">
            ...
            @php $what_to_bring = $tour->translate('what_to_bring'); @endphp
            @if(!empty($what_to_bring) && is_array($what_to_bring))
            <div class="mb-10">
                <h2 class="font-display text-2xl font-semibold text-gray-900 mb-4">{{ __('What to Bring') }}</h2>
                <ul class="grid grid-cols-1 md:grid-cols-2 gap-3">
                    @foreach($what_to_bring as $item)
                        @if($item)
                        <li class="flex items-center gap-3 text-gray-600">
                            <span class="w-2 h-2 bg-gold-500 rounded-full flex-shrink-0"></span>
                            <span>{{ is_array($item) ? implode(', ', $item) : __($item) }}</span>
                        </li>
                        @endif
                    @endforeach
                </ul>
            ...
            @php $good_to_know = $tour->translate('good_to_know'); @endphp
            @if(!empty($good_to_know) && is_array($good_to_know))
            <div class="mb-10">
                <h2 class="font-display text-2xl font-semibold text-gray-900 mb-4">{{ __('Good to Know') }}</h2>
                <ul class="space-y-3">
            ...
            <div class="mb-10">
                <h2 class="font-display text-2xl font-semibold text-gray-900 mb-6">{{ __('Frequently Asked Questions') }}</h2>
                <div class="space-y-3">
            ...
            <div class="mb-16 bg-gray-50 rounded-[2.5rem] p-8 md:p-12 border border-gray-100 shadow-sm" x-data="{
                ...
                <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                    <div>
                        <h2 class="font-display text-3xl font-black text-safari-dark uppercase tracking-tight">{{ __('Meeting Points') }}</h2>
                        <p class="text-gray-500 text-sm mt-2 font-medium">{{ __('We offer flexible arrival options. Explore our meeting points below; you can confirm your choice during booking.') }}</p>
                    </div>
                ...
                <div class="bg-white p-2 rounded-[2rem] border border-gray-200 shadow-inner overflow-hidden h-[400px] relative">
                    ...
                    <div class="absolute bottom-6 left-1/2 -translate-x-1/2 bg-safari-dark/90 backdrop-blur-md px-6 py-3 rounded-full border border-white/10 shadow-2xl">
                        <p class="text-[10px] font-black text-white uppercase tracking-[0.2em] whitespace-nowrap">
                            {{ __('Viewing Location') }}: <span class="text-gold-400" x-text="points[activePoint].name"></span>
                        </p>
                    </div>
                </div>
            </div>
            ...
            <div id="reviews" class="mb-10 pt-10 border-t border-gray-100">
                <div class="flex items-center justify-between mb-8">
                    <div>
                        ...
                        <div class="flex items-center gap-2 mt-2">
                            ...
                            <span class="text-gray-500 text-sm">({{ $tour->review_count }} {{ __('verified reviews') }})</span>
                        </div>
                    </div>
                ...
                <div id="review-form-container" class="hidden mb-12 bg-gray-50 rounded-3xl p-8 border border-gray-100">
                    <h3 class="text-xl font-bold text-gray-900 mb-6">{{ __('Share Your Experience') }}</h3>
                    <form action="{{ route('reviews.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">{{ __('Full Name') }}</label>
                        ...
                        <div class="mb-6">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">{{ __('Rating') }}</label>
                        ...
                        <div class="mb-6">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">{{ __('Review Title') }}</label>
                            <input type="text" name="title" placeholder="{{ __('Summarize your experience') }}" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-gold-500/20 outline-none">
                        </div>

                        <div class="mb-6">
                            <label class="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">{{ __('Your Review') }}</label>
                            <textarea name="content" rows="5" required placeholder="{{ __('What did you love most about this trip?') }}" class="w-full bg-white border border-gray-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-gold-500/20 outline-none"></textarea>
                        </div>

                        <button type="submit" class="w-full btn-gold py-4 rounded-xl font-bold shadow-lg">{{ __('Submit Review') }}</button>
                    </form>
                </div>
                ...
                <div class="space-y-6">
                            ...
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-gray-400">{{ $review->country ?? __('Verified Traveler') }} · {{ $review->created_at->format('M Y') }}</p>
                                    </div>
                                </div>
                                <div class="flex text-gold-500">
                                    @for($i=1; $i<=5; $i++)
                    ...
                    @empty
                        <div class="text-center py-10 bg-gray-50 rounded-3xl border border-dashed border-gray-200">
                            <p class="text-gray-500">{{ __('No reviews yet. Be the first to share your experience!') }}</p>
                        </div>
                    @endforelse
                </div>
            </div>
            ...
            <div class="sticky top-24 space-y-4">
                <div class="bg-safari-dark text-white rounded-2xl p-6 shadow-xl">
                    <div class="text-center mb-6">
                        <div class="text-xs text-gray-400 uppercase tracking-wider mb-1">{{ __('Starting from') }}</div>
                        <div class="font-display text-4xl font-bold text-gold-400">{{ $tour->formatted_price }}</div>
                        <div class="text-gray-400 text-sm">{{ __($tour->price_note ?? 'per person (sharing)') }}</div>
                    </div>
                    ...
                    <div class="mt-4 pt-4 border-t border-white/10 space-y-3 text-sm">
                        <div class="flex items-center gap-3 text-gray-300">
                            ...
                            <span>{{ __('Free cancellation (30 days)') }}</span>
                        </div>
                        <div class="flex items-center gap-3 text-gray-300">
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6">
                    <h4 class="font-semibold text-gray-800 mb-4 uppercase tracking-wider text-xs">{{ __('Tour Details') }}</h4>
                    <div class="space-y-3">
                        <div class="flex justify-between text-sm border-b border-gray-50 pb-2"><span class="text-gray-400">{{ __('Duration') }}</span><span class="font-bold text-gray-700">{{ $tour->duration_text }}</span></div>
                        <div class="flex justify-between text-sm border-b border-gray-50 pb-2"><span class="text-gray-400">{{ __('Group Size') }}</span><span class="font-bold text-gray-700">{{ $tour->group_size_min ? $tour->group_size_min : '1' }}-{{ $tour->group_size_max ? $tour->group_size_max : '12' }} {{ __('people') }}</span></div>
                        @if($tour->difficulty_level)
                        <div class="flex justify-between text-sm border-b border-gray-50 pb-2"><span class="text-gray-400">{{ __('Difficulty') }}</span><span class="font-bold text-gray-700 capitalize">{{ __($tour->difficulty_level) }}</span></div>
                        @endif
                        @if($tour->accommodation_type)
                        <div class="flex justify-between text-sm border-b border-gray-50 pb-2"><span class="text-gray-400">{{ __('Accommodation') }}</span><span class="font-bold text-gray-700">{{ __($tour->accommodation_type) }}</span></div>
                        @endif
                        @if($tour->departure_location)
                        <div class="flex justify-between text-sm border-b border-gray-50 pb-2"><span class="text-gray-400">{{ __('Departure') }}</span><span class="font-bold text-gray-700">{{ __($tour->departure_location) }}</span></div>
                        @endif
                        @if($tour->departure_time)
                        <div class="flex justify-between text-sm border-b border-gray-50 pb-2"><span class="text-gray-400">{{ __('Departure Time') }}</span><span class="font-bold text-gray-700">{{ $tour->departure_time }}</span></div>
                        @endif
                        @if($tour->min_age)
                        <div class="flex justify-between text-sm border-b border-gray-50 pb-2"><span class="text-gray-400">{{ __('Min Age') }}</span><span class="font-bold text-gray-700">{{ $tour->min_age }}+ {{ __('years') }}</span></div>
                        @endif
                        @if(!empty($tour->languages_offered) && is_array($tour->languages_offered))
                        <div class="flex justify-between text-sm border-b border-gray-50 pb-2"><span class="text-gray-400">{{ __('Languages') }}</span><span class="font-bold text-gray-700">{{ implode(', ', $tour->languages_offered) }}</span></div>
                        @endif
                    </div>
                ...
                <div class="bg-gold-50 border border-gold-200 rounded-2xl p-5 mb-4">
                    <p class="text-sm text-gray-600 mb-3">{{ __('Need help planning? Talk to our safari experts.') }}</p>
                ...
                <div class="bg-white rounded-2xl border border-gray-100 p-6 shadow-sm">
                    <h4 class="font-bold text-gray-900 mb-4">{{ __('Ask about this Tour') }}</h4>
                    <form action="{{ route('tours.inquiry', $tour->id) }}" method="POST" class="space-y-4">
                        @csrf
                        <input type="text" name="name" required placeholder="{{ __('Your Name') }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-gold-500">
                        <input type="email" name="email" required placeholder="{{ __('Email Address') }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-gold-500">
                        <textarea name="message" rows="3" required placeholder="{{ __('What would you like to know?') }}" class="w-full bg-gray-50 border border-gray-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-gold-500 resize-none"></textarea>
    ...
    <div class="mt-16">
        <h2 class="font-display text-3xl font-semibold text-gray-900 mb-8">{{ __('You Might Also Like') }}</h2>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div class="tour-card bg-white rounded-2xl overflow-hidden border border-gray-100 shadow-sm">
                <div class="img-zoom h-44"><img src="{{ $related->featured_image_url }}" width="400" height="300" alt="{{ $related->translate('title') }}" class="w-full h-full object-cover" loading="lazy"></div>
                <div class="p-4">
                    <h4 class="font-semibold text-gray-800 text-sm mb-1 line-clamp-2"><a href="{{ route('tours.show', ['type' => $related->item_type ?? 'tour', 'slug' => $related->slug ?? 'default']) }}" class="hover:text-gold-600 transition-colors">{{ $related->translate('title') }}</a></h4>
